...
class SharedPreferencesGlobal {
  ...
  static dynamic _handleType<T>(final String key, {final value}) {
    switch (T) {
      case const (String):
        return value != null
  ...
  static Future<void> setValue<T>(final String key, final T value) async {
    await _handleType<T>(key, value: value);
  }

  static T getValue<T>(final String key) => _handleType<T>(key);

  static Future<void> editValue<T>(
    final String key,
  ...
  static Future<void> deleteValue(final String key) async {
    await _prefs.remove(key);
  }

  static bool isContainsKey(final String key) => _prefs.containsKey(key);
  ...
  static Future<void> clearDataInKey(final String key) async {
    await _prefs.remove(key);
  }
}
